<template>
  <div class="container mx-auto bg-gray-800 mt-12 shadow-2xl rounded-3xl pb-5">
    <div class="flex px-8 flex-wrap">
      ...
      <div class="flex justify-end py-5 w-full" v-if="!checkProjectId">
        <button
          @click="SaveData"
          class="
            bg-blue-500
      ...
      <div class="flex justify-end py-5 w-full" v-else>
        <button
          @click="updateData"
          class="
            bg-yellow-500
...
export default {
  ...
  methods: {
    ...
    SaveData() {
      ...
      uploadBytes(storageRef, this.authorImg)
        .then((snapshot) => {
        ...
        uploadBytes(storageRef, this.authorImg)
          ...
          .then(() => {
            ...
            getDownloadURL(ref(storage, this.url))
              .then((url) => {
                ...
                updateDoc(selectedDocument, {
                  project_name: this.projectName,
                  project_description: this.projectDescription,
                  img_url: url ? url : this.authorImagePaint,
                });
                this.clearFrom();
              })
              .catch((error) => {
            ...
            this.$toast.success(`Hey! I'm here`, {
              ...
              dismissible: false,
              duration: 4000,
              message: `პროექტი წარმატებით დარედაქტირდა!`,
            });
            this.clearFrom();
          })
          .catch((error) => {
